...
v3.4.0 已完成。v3.2.1 的查询下界只适用于 v3.1 的 8 个允许格点；v3.4 将 v3.3 的单个分支表推广到运行区间—局部响应的参数化有限黑箱族，并在冻结的参数留出配置中复现优势。当前边界仍是小型、无噪声、显式枚举的候选系统，不代表通用系统识别或主动学习性能。
...
所有测试、端到端验收、v3.0 至 v3.4 基准、审计和清单均可重放。v3.1 只胜过随机采样；v3.2.1 证明该线性任务中固定与自适应的下界同为 2；v3.3 给出严格优势的存在性证据；v3.4 在两个参数留出配置中复现优势：4 个运行区间、2 个响应时为 2 对 5，3 个运行区间、3 个响应时为 2 对 4；单响应留出控制为 1 对 1。
...
- 最新版本研究反思：`RESEARCH_REFLECTION_v3_4.md`
