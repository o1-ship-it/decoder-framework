...
v3.5.0 已完成。v3.4 的运行区间—局部响应结构现由二维分段整数映射规则直接求值生成，并在冻结的映射参数留出配置中复现优势。当前边界仍是小型、无噪声、显式枚举的候选系统，不代表通用系统识别或主动学习性能。
...
所有测试、端到端验收、v3.0 至 v3.5 基准、审计和清单均可重放。v3.5 在二维分段整数映射族的两个留出配置中复现优势：4 个区域、2 个响应时为 2 对 5，3 个区域、3 个响应时为 2 对 4；单响应控制为 1 对 1。候选转移由映射求值产生，不再直接作为输入表提供。
