...
v3.1 已完成。v3.0 已完成并推送，提交为 `03654da`。当前边界是二维整数、低次、无噪声映射，不代表通用系统识别。
...
所有测试、端到端验收、v3.0 基准、审计和清单均已通过。v3.1 证明了随机采样的效率差异，但与合适固定设计平局；下一阶段必须同强非自适应基线比较，不能把随机基线优势宣传为主动学习优势。
